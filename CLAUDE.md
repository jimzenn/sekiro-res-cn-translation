...
- **来源取舍**（已修订，见 MEMORY.md "JP 实为高质量底本" 一节）：**一律以 `jp` 为优先源，`en` 仅作参考**。
  - 原版相关内容：官方中文本就译自日文，jp 是权威。
  - Resurrection 新增内容：mod 的 JP 由 co-developer **TotallyNotShinobi（Mikiri）** 操刀，文体深度模仿 From 笔法（古风助动词「ねば／ぬ／おる／なかろうて／しからば」、体言止め+留白、提问—反讽叙事），质量普遍高于 EN。EN 一侧常见扩写、口语化、打破第四面墙、与 JP 语义错位（例：「お主、平田の忍びだな」→ "You must be on our side." 完全跑题；「奴らの親玉が残っておる」→ "There's still the big guy over there." 失态），**不可作唯一依据**。
  - **唯一例外**：`jp` 为 `null` 的少数条目（约 15 条，如 `Training Dojo` / `The Tengu of Ashina`），mod 本身没出日文，照 `en` 翻即可。
  - `jp` 与 `en` 含义对不上时**永远偏 jp**。
- **务必原样保留**：占位标签如 `<?goodsItemNum@4300?>`、换行 `\n`、全角空格 `　`(`　`)、以及任何格式控制符——别翻译、别删、别改位置。
- 描述类长文本注意**行宽与换行**，参考同类 `from_improved` 词条的断行习惯，避免单行过长在游戏里溢出。
- **括号 / 引号规约**：UI 标签 / 状态术语 / 系统提示统一用 `［］`（全角方括号 U+FF3B/D）；引用、对白引用、quest 名、嵌入叙述的物品/招式名用 `「」`（直角引号）。**不要使用 `【】`**——这对括号源自日本字典词头墨付括弧（すみつきかっこ），视觉过重且语义角色混乱，本项目已统一退役（连 `from_improved` 基准条目也一并替换）。`cn_improved` 参照字段里的 `【】` 是历史记录，请勿动。
